...
export function countOccurrences(data, letter) {
    let matches = data.match(new RegExp(letter, "g"));
...
function isValidPassword(value) {
    ...
    let letter = split[2]
    let occurrence = countOccurrences(password, letter)

    if (occurrence >= split[0] && occurrence <= split[1]) {
        return true;
    }
    return false;
}
