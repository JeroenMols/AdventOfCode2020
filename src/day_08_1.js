...
function findDuplicateInstruction(code, accStart) {
    let index = 0
    let runProgram = true
    let instruction, amount
    while (runProgram) {
        instruction = code[index]
        console.log("Executing " + index + " => " + instruction)
        amount = parseInt(instruction.match(/[+-]\d+/)[0])
        ...
        if (executedInstructions.has(index)) {
            runProgram = false
        } else {
...
console.log(findDuplicateInstruction(testValues, 0))
